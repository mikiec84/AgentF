package starling.utils
{
    ...
    public class AssetManager
    {
        ...
        public function enqueue(...rawAssets):void
        {
            for each (var rawAsset:Object in rawAssets)
            {
                ...
                else if (rawAsset is Class)
                {
					...
					//Added by Anton Licht:
					for each (childNode in typeXml.factory.extendsClass.(@type == "flash.display::BitmapData"))
						push(rawAsset,(typeXml.@name).split("::").pop());
					//--------------------
                                      ...
                                      onProgress:Function, onComplete:Function):void
        {
            ...
            {
				var asset:Object;
				var isBitmapData:Boolean = false;
				var classXML:XML = describeType(rawAsset as Class).factory[0];
				for each (var b:XML in classXML.extendsClass.(@type == "flash.display::BitmapData"))
				{
					isBitmapData = true;
					break;
				}
				if(isBitmapData)
                	asset = new (rawAsset as Class)(0,0);
				else
					asset = new (rawAsset as Class)();
            ...
            function onLoaderComplete(event:Event):void
            {
                ...
                if (content is Bitmap)
                    addTexture(name, Texture.fromBitmap(content as Bitmap, mUseMipMaps, false, mScaleFactor));
